...
void Text_DrawRect(unsigned int texture, Rect2D screenRect, Rect2D imgRect){
    ...
    for(k = 0; k < 6; k++){
        ...
        uv.x = (squareData[k].coord.x * imgRect.w) + imgRect.x;
        uv.y = (squareData[k].coord.y * imgRect.h) + imgRect.y;
        pos.x = ((squareData[k].pos.x+0.5) * screenRect.w) + screenRect.x;
        pos.y = ((squareData[k].pos.y+0.5) * screenRect.h) + screenRect.y;

        glBindBuffer(GL_ARRAY_BUFFER, positionVbo);
        glBufferSubData(GL_ARRAY_BUFFER, sizeof(Vec2) * k, sizeof(Vec2), &pos.x);
        glBindBuffer(GL_ARRAY_BUFFER, uvVbo);
        glBufferSubData(GL_ARRAY_BUFFER, sizeof(Vec2) * k, sizeof(Vec2), &uv.x);
    }
