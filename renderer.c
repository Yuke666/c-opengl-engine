...
static void NormalDepthPrepass_Resize(NormalDepthPrepass *ndPrepass, int resX, int resY){
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH24_STENCIL8, resX, resY, 0, GL_DEPTH24_STENCIL8, GL_UNSIGNED_INT_24_8, 0);
...
static void NormalDepthPrepass_Init(NormalDepthPrepass *ndPrepass, int resX, int resY){
	...
	GenerateColorTex(resX, resY, &ndPrepass->normalTexture, GL_LINEAR, GL_RGBA16F, GL_RGBA, GL_FLOAT);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, ndPrepass->normalTexture, 0);

	GenerateDepthTex(resX, resY, &ndPrepass->depthTexture, GL_DEPTH24_STENCIL8, GL_DEPTH_STENCIL, GL_UNSIGNED_INT_24_8, GL_LINEAR);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_TEXTURE_2D, ndPrepass->depthTexture, 0);
