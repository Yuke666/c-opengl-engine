...
void main(){
	...
	// lightness = length(sobel) > 0.5 ? 1 : 0;
	...
	random.xy = texture2D(randomTexture, TexCoord * noiseScale).xy;
	random.z = 0;
	...
	for(int k = 0; k < KERNEL_SIZE; k++){
		...
		float depth = ScreenSpaceToViewSpaceZ(((offset.xy/offset.w) * 0.5) + 0.5);

		// vec3 final = ScreenSpaceToViewSpace(((offset.xy/offset.w) * 0.5) + 0.5);

		float rangeCheck = smoothstep(0.0, 1.0, radius / abs(-pos.z + depth));
		occlusion += (depth >= sample.z ? 1.0 : 0.0) * rangeCheck;
	
		// occlusion = abs(pos.z - depth) > radius ? 1 : 0;

		// occlusion += -pos.z + depth >= radius ? 1 : 0;
	}
	...
	// gl_FragColor = vec4(vec3(length(pos - final)),1);
	// gl_FragColor = vec4(vec3(-pos.z + final.z),1);
}